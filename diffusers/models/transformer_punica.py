...
from .attention_punica import BasicTransformerBlock
...
from punica.utils import BatchedKvCache, BatchedLoraWeight, BatchLenInfo, LoraWeight, BatchedLlamaLoraWeight
    ...
    def forward(
        ...
        if self.is_input_continuous:
            ...
            if not self.use_linear_projection:
                ...
                inner_dim = hidden_states.shape[1]
                hidden_states = hidden_states.permute(0, 2, 3, 1).reshape(batch, height * width, inner_dim)
                with torch.cuda.nvtx.range("in_proj"):
                    hidden_in = (
                        self.proj_in(hidden_states, scale=lora_scale)
                        if not USE_PEFT_BACKEND
                        else self.proj_in(hidden_states)
                    )
                with torch.cuda.nvtx.range("lora_in"):
                    add_lora(
                        hidden_in,
                        hidden_states,
                        lora.up.wa_ptr,
